...
//        online_order_email = findViewById(R.id.online_order_email);
    ...
    private void sendFeedbackProvided(String fd) {
        final ProgressDialog dialog = new ProgressDialog(BookDetails.this);
        dialog.setMessage("Please wait...");
        dialog.show();

//        final String ff = feedback_provided.getText().toString().trim();
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, urls.SEND_FEEDBACK,
                response -> {
                    try {
                        ...
                        if (success.equals("1")) {
                            dialog.dismiss();
//                            Toast.makeText(BookDetails.this, "Feedback sent successfully", Toast.LENGTH_LONG).show();
...
//                                    finish();
                        ...
                        e.printStackTrace();
//                        Toast.makeText(BookDetails.this, "Feedback not sent successful, please try again ", Toast.LENGTH_LONG).show();
                        dialog.dismiss();
                        ee_card = new Dialog(this);
...
//            Toast.makeText(BookDetails.this, "Feedback not sent successful, please check your network and try again", Toast.LENGTH_LONG).show();
            dialog.dismiss();
            ee_card = new Dialog(this);
            ee_card.setContentView(R.layout.error);
            CardView dismiss = ee_card.findViewById(R.id.ee_card);
...
type id = 1 thats for soft copy orders
    */
    private void sendEmailProvided(String oo) {
        ...
        final String book_titles = book_title.getText().toString().trim();
        final String bb = book_id.getText().toString().trim();
